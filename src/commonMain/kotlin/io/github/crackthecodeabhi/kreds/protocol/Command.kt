/*
 *  Copyright (C) 2022 Abhijith Shivaswamy, Ido Flax
...
internal fun Command.toRedisMessageList(): List<FullBulkStringRedisMessage> {
    return if (subCommand != null) {
        listOf(FullBulkStringRedisMessage(string.toBuffer()), *subCommand!!.toRedisMessageList().toTypedArray())
    } else listOf(FullBulkStringRedisMessage(string.toBuffer()))
}
...
internal interface ICommandProcessor<out R> {
    fun encode(command: Command, vararg args: Argument): RedisMessage {
        ...
        x.addAll(args.filter { it !is EmptyArgument }.map { FullBulkStringRedisMessage(it.toString().toBuffer()) })
...
internal val AllCommandProcessor =
    CommandProcessor<Any?>(SimpleStringHandler, IntegerHandler, BulkStringHandler, ArrayHandler)
